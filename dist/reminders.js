...
function hourIn(timezone, at = new Date()) {
    try {
        ...
        return Number(new Intl.DateTimeFormat('en-GB', { timeZone: DEFAULT_TIMEZONE, hour: '2-digit', hourCycle: 'h23' }).format(at));
    }
}
...
async function runRemindersOnce() {
    ...
    for (const row of rows) {
        if (!row.telegram_user_id || !isDue(row))
            continue;
        if (isQuietHour(hourIn(timezoneFor(row.phone))))
...
function startReminders() {
    console.log('[reminder] scheduler started | every', RUN_EVERY_MS / 60000, 'minutes');
