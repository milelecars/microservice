...
async function getLead(kommoLeadId) {
    try {
        const resp = await axios_1.default.get(`${SUPABASE_URL}/rest/v1/leads?kommo_lead_id=eq.${kommoLeadId}&limit=1`, { headers, timeout: 10000 });
...
async function updateLead(kommoLeadId, changes) {
    if (Object.keys(changes).length === 0) {
        console.log('[supabase] no changes for lead:', kommoLeadId, '— skipping');
        return;
    }
    try {
        const resp = await axios_1.default.patch(`${SUPABASE_URL}/rest/v1/leads?kommo_lead_id=eq.${kommoLeadId}`, changes, { headers: { ...headers, 'Prefer': 'return=minimal' }, timeout: 10000 });
